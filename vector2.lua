-- Modified from https://github.com/AIUbi/2D-vector-for-Lua/blob/master/vector2.lua
...
META =
{
	...
	angle_atan = function(lhs)	return math.deg(math.atan2(lhs.x, lhs.y)) end,
	angle_cos = function(lhs, vec)	return math.deg(math.acos(lhs:normalize():dot_product(vec:normalize()))) end,
...
-- edited
vec2 = function(x, y) return setmetatable({x = x, y = y}, META) end
